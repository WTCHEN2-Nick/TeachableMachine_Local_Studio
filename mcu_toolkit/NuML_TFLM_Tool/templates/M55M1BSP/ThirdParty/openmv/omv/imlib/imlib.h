...
#ifndef __UXTB16_RORn
#define __UXTB16_RORn(ARG1, ARG2)        __UXTB16(__ROR(ARG1, ARG2))
#endif
...
({ \
    uint32_t _pixel = (pixel); \
    color_thresholds_list_lnk_data_t *_threshold = (threshold); \
    bool _invert = (invert); \
...
#define IM_EQUAL(img0, img1) \
    ({ (img0->w==img1->w)&&(img0->h==img1->h)&&(img0->pixfmt=img1->pixfmt); })
...
//modify by chchen59
float imlib_template_match_ds(image_t *image, image_t *template_img, rectangle_t *r);
float imlib_template_match_ex(image_t *image, image_t *template_img, rectangle_t *roi, int step, rectangle_t *r);
