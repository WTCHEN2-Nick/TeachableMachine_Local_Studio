...
/* _sd_uR3_CMD is the flag for it. 1 means software should ignore CRC-7 error */
    ...
    #define __SCB_DCACHE_LINE_SIZE    (32UL)
#endif
...
/* Preserve the legacy reserved high nibble together with the documented writable fields. */
#define SDH_CTL_WRITE_KEEP_MASK    (0xF0000000U | SDH_CTL_SDNWR_Msk | SDH_CTL_DBW_Msk | SDH_CTL_CLKKEEP_Msk)

static uint32_t SDH_GetDMAAddress(const void *pvBuf)
{
    uintptr_t uAddr = (uintptr_t)pvBuf;

    return (uint32_t)uAddr;
}

static uint32_t SDH_IsValidInstance(const SDH_T *sdh)
{
    return (((sdh == SDH0) || (sdh == SDH1)) ? 1U : 0U);
}

static void SDH_InvalidateCardState(SDH_INFO_T *pSD)
{
    uint8_t *pu8DMABuf;

    if (pSD == (SDH_INFO_T *)NULL)
    {
        return;
    }

    pu8DMABuf = pSD->dmabuf;

    (void)memset(pSD, 0, sizeof(SDH_INFO_T));
    pSD->dmabuf = pu8DMABuf;
    pSD->CardType = SDH_TYPE_UNKNOWN;
}

static uint32_t SDH_EnsureCardReady(SDH_T *sdh)
{
    const SDH_INFO_T *pSD;

    pSD = SDH_GetSDInfoMsg(sdh);
    ...
    if (SDH_CardDetection(sdh) == FALSE)
    {
        return SDH_NO_SD_CARD;
    }

    if ((pSD->CardType == SDH_TYPE_UNKNOWN) || (pSD->RCA == 0U))
    {
        return SDH_Probe(sdh);
    }

    return Successful;
}

static void SDH_UpdateDataReadyFlag(SDH_T *sdh, SDH_INFO_T *pSD)
{
    ...
    if ((sdh->INTSTS & SDH_INTSTS_BLKDIF_Msk) != 0U)
    {
        sdh->INTSTS = SDH_INTSTS_BLKDIF_Msk;
        pSD->DataReadyFlag = (uint8_t)TRUE;
    }
}

#if (NVT_DCACHE_ON == 1)
static void SDH_InvalidateDCacheBuffer(const void *pvBuf, uint32_t u32Size)
{
    uintptr_t u32StartAddr = (uintptr_t)pvBuf;
    uintptr_t u32AlignedStart;
    uintptr_t u32AlignedEnd;
    int32_t u32CacheSize;
    ...
    u32AlignedStart = u32StartAddr & ~((uintptr_t)DCACHE_LINE_SIZE - 1U);
    u32AlignedEnd = (u32StartAddr + (uintptr_t)u32Size + (uintptr_t)DCACHE_LINE_SIZE - 1U) &
                    ~((uintptr_t)DCACHE_LINE_SIZE - 1U);
    u32CacheSize = (int32_t)u32AlignedEnd - (int32_t)u32AlignedStart;

    SCB_InvalidateDCache_by_Addr((void *)u32AlignedStart, u32CacheSize);
}

static void SDH_CleanDCacheBuffer(const void *pvBuf, uint32_t u32Size)
{
    uintptr_t u32StartAddr = (uintptr_t)pvBuf;
    uintptr_t u32AlignedStart;
    uintptr_t u32AlignedEnd;
    int32_t i32CacheSize;
    ...
    u32AlignedEnd = (u32StartAddr + (uintptr_t)u32Size + (uintptr_t)DCACHE_LINE_SIZE - 1U) &
                    ~((uintptr_t)DCACHE_LINE_SIZE - 1U);
    i32CacheSize = (int32_t)u32AlignedEnd - (int32_t)u32AlignedStart;

    SCB_CleanDCache_by_Addr((void *)u32AlignedStart, i32CacheSize);
}
#endif
...
{
    static uint32_t s_u32SDH0ClkSrc;
    static uint32_t s_u32SDH1ClkSrc;
    ...
    if (u32SD_Clk_khzTemp <= 400U)
    {
        ...
        if (sdh == SDH0)
        {
            s_u32SDH0ClkSrc = (CLK->SDHSEL & CLK_SDHSEL_SDH0SEL_Msk);
        ...
        {
            s_u32SDH1ClkSrc = (CLK->SDHSEL & CLK_SDHSEL_SDH1SEL_Msk);
    ...
    else
    {
        ...
        if (sdh == SDH0)
        {
            CLK->SDHSEL = (CLK->SDHSEL & ~CLK_SDHSEL_SDH0SEL_Msk) | s_u32SDH0ClkSrc;

            switch (s_u32SDH0ClkSrc)
            {
        ...
        {
            CLK->SDHSEL = (CLK->SDHSEL & ~CLK_SDHSEL_SDH1SEL_Msk) | s_u32SDH1ClkSrc;

            switch (s_u32SDH1ClkSrc)
            {
...
uint32_t SDH_SwitchToHighSpeed(SDH_T *sdh, SDH_INFO_T *pSD)
{
    ...
    if ((SDH_IsValidInstance(sdh) == 0U) ||
            (pSD == (SDH_INFO_T *)NULL) ||
            (((sdh == SDH0) && (pSD != &SD0)) || ((sdh == SDH1) && (pSD != &SD1))) ||
            (pSD->dmabuf == (uint8_t *)NULL))
    {
        return SDH_SELECT_ERROR;
    }
    ...
    sdh->DMASA = SDH_GetDMAAddress(pSD->dmabuf);
    sdh->BLEN = 63U;
...
static uint32_t _SDH_Read(SDH_T *sdh, const uint8_t *pu8BufAddr, uint32_t u32StartSec, uint32_t u32SecCount)
{
    ...
    sdh->DMASA = SDH_GetDMAAddress(pu8BufAddr);
    ...
    for (i = 0U; i < u32Loop; i++)
    {
        ...
        {
            SDH_UpdateDataReadyFlag(sdh, pSD);
...
uint32_t SDH_Read(SDH_T *sdh, uint8_t *pu8BufAddr, uint32_t u32StartSec, uint32_t u32SecCount)
{
    ...
    u32DrvRet = SDH_EnsureCardReady(sdh);
    ...
    if (((uintptr_t)pu8BufAddr % (uintptr_t)DEF_ALIGNED_VALUE) != 0UL)
    {
        const uint8_t *pu8NCBuffer;
        uint32_t u32i = 0;
...
#if (NVT_DCACHE_ON == 1)
        SDH_InvalidateDCacheBuffer((void *)pu8BufAddr, u32SecCount * SDH_BLOCK_SIZE);
#endif
    }
...
static uint32_t _SDH_Write(SDH_T *sdh, const uint8_t *pu8BufAddr, uint32_t u32StartSec, uint32_t u32SecCount)
{
    uint32_t volatile u32BIsSendCmd = FALSE;
    uint32_t volatile u32Reg;
    ...
    {
        pSD->DataReadyFlag = (uint8_t)FALSE;
        u32Reg = (sdh->CTL & SDH_CTL_WRITE_KEEP_MASK) | SDH_CTL_BLKCNT_Msk;   /* set BLK_CNT to 0xFF=255 */

        if (!u32BIsSendCmd)
        {
            sdh->CTL = (u32Reg |
                        ((uint32_t)25U << SDH_CTL_CMDCODE_Pos) |
                        (SDH_CTL_COEN_Msk |
                         SDH_CTL_RIEN_Msk |
                         SDH_CTL_DOEN_Msk));
            u32BIsSendCmd = TRUE;
    ...
    {
        pSD->DataReadyFlag = (uint8_t)FALSE;
        u32Reg = (sdh->CTL & SDH_CTL_WRITE_KEEP_MASK) | ((u32Loop << SDH_CTL_BLKCNT_Pos) & SDH_CTL_BLKCNT_Msk);
...
uint32_t SDH_Write(SDH_T *sdh, uint8_t *pu8BufAddr, uint32_t u32StartSec, uint32_t u32SecCount)
{
    ...
    {
#if (NVT_DCACHE_ON == 1)
        SDH_CleanDCacheBuffer((void *)pu8BufAddr, u32SecCount * SDH_BLOCK_SIZE);
#endif
