...
#define OTFC_OK                         ( 0)   /*!< OTFC operation OK */
#define OTFC_ERR_FAIL                   (-1)   /*!< OTFC operation failed */
#define OTFC_ERR_TIMEOUT                (-2)   /*!< OTFC operation abort due to timeout error */

/*----------------------------------------------------------------------------*/
/* OTFC_CTL constant definitions                                              */
/*----------------------------------------------------------------------------*/
#define OTFC_CTL_PR_Pos                 (8U)     /*!< OTFC CTL Protection Region BIT SHIFT */

#define OTFC_CTL_RST_Pos                (1U)
#define OTFC_CTL_SWAP_Pos               (2U)
#define OTFC_CTL_INT_Pos                (7U)

#define OTFC_STS_KSERR_Pos(pr)          (((pr) * OTFC_CTL_PR_Pos) + 5)
#define OTFC_STS_KSERR_Msk(pr)          ((uint32_t)0x1U << OTFC_STS_KSERR_Pos(pr))

#define OTFC_STS_DFAE_Pos(pr)           (((pr) * OTFC_CTL_PR_Pos) + 6)
#define OTFC_STS_DFAE_Mak(pr)           ((uint32_t)0x1U << OTFC_STS_DFAE_Pos(pr))
...
#define OTFC_KS_SRC_SRAM                (0x0U)    /*!< Key is read from the SRAM of Key Store. */
#define OTFC_KS_SRC_OTP                 (0x2U)    /*!< Key is read from the OTP of Key Store. */
...
#define OTFC_RESET_PR(otfc, pr) \
    ((otfc)->CTL |= ((uint32_t)0x1U << (((uint32_t)(pr) * OTFC_CTL_PR_Pos) + OTFC_CTL_RST_Pos)))
...
#define OTFC_ENABLE_KEYSWAP(otfc, pr)  \
    ((otfc)->CTL |= ((uint32_t)0x1U << (((uint32_t)(pr) * OTFC_CTL_PR_Pos) + OTFC_CTL_SWAP_Pos)))
...
#define OTFC_ENABLE_INT(otfc, pr)   \
    ((otfc)->CTL |= ((uint32_t)0x1U << (((uint32_t)(pr) * OTFC_CTL_PR_Pos) + OTFC_CTL_INT_Pos)))
...
#define OTFC_GET_BUSY(otfc, pr)   \
    (((otfc)->STS & ((uint32_t)0x1U << ((uint32_t)(pr) * OTFC_CTL_PR_Pos))) >> ((uint32_t)(pr) * OTFC_CTL_PR_Pos))
...
#define OTFC_SET_START_ADDR(otfc, pr, saddr)    \
    (otfc)->PR[(pr)].SADDR = (uint32_t)((uint32_t)(saddr) & (uint32_t)0xFFFFFFF0UL)
...
#define OTFC_SET_END_ADDR(otfc, pr, eaddr)  \
    ((otfc)->PR[(pr)].EADDR = (uint32_t)((uint32_t)(eaddr) & (uint32_t)0xFFFFFFF0UL))
...
/** @addtogroup OTFC_EXPORTED_FUNCTIONS OTFC Exported Functions
  @{
*/
int32_t OTFC_SetKeyFromKeyReg(OTFC_T *otfc, const uint32_t *pau32KeyTable, uint32_t u32PR, uint32_t u32SAddr, uint32_t u32PRSize);
int32_t OTFC_SetKeyFromKeyStore(OTFC_T *otfc, uint32_t u32PR, uint32_t u32SAddr, uint32_t u32PRSize, uint32_t u32KeyNum, uint32_t u32KeySrc);
