...
void UVC_Init(void)
{
    ...
    HSUSBD_ENABLE_USB_INT(HSUSBD_GINTEN_USBIEN_Msk | HSUSBD_GINTEN_CEPIEN_Msk | HSUSBD_GINTEN_EPAIEN_Msk | HSUSBD_GINTEN_EPCIEN_Msk | NuML_CDC_GlobalIntMask());
...
void UVC_ClassRequest(void)
{
    ...
    if (gUsbCmd.bmRequestType & 0x80)
    {
        ...
        else if (NuML_CDC_ClassRequest())
        {
            /* Handled by the CDC function. */
        }
        else
        {
...
{
    /* NuML_CDC_SETINTERFACE_GUARD: only the VideoStreaming interface
     * may drive gAppConnected.  Without this a CDC SET_INTERFACE would
     * silently stop the video stream. */
    if ((gUsbCmd.wIndex & 0xfful) != 0x01ul)
    {
        return;
    }
...
bool UVC_IsConnect(void)
{
    if ((gAppConnected) && (HSUSBD_IS_ATTACHED()))
        return TRUE;
    return FALSE;
}
